...
phenos = data.frame()
for (p in unique(inf_phenos$Phenotype)) {
  id <- get_hpo_termID(p, phenotype_to_genes)
  ontLvl_geneCount_ratio <- (ont_level + 1)/length(get_gene_list(p,phenotype_to_genes))
...
```{r plot, warning = FALSE, message = FALSE, fig.width = 1000, fig.height  = 1000}
...
```{r interactive, warning=FALSE , message=FALSE, fig.width=1000, fig.height=1000}
if (!require("plotly")) {
